...
class my_validation:
    def __init__(self, filename, sheet, merge_on, check_on):
        ...
        self.dataframe = (
            pd.read_excel(
                self.file_path, sheet_name=sheet, dtype={merge_on: str, check_on: float}
            )
    ...
    def odtl_check(self, column_1, column_2, filename):
        #self[column_2] = self[column_2].astype(float)
        if self[self[column_1] != self[column_2]].shape[0] : #in range(0, 5):
            print("        balances are all matched.")
            return self
        else:
            print("        Not matched; Balance and M_Bnm_Balance are not matched")
            self.to_excel(
                my_path
                + r"\logs\log_file(balances_not_matched) for ("
                + filename
                + ").xlsx",
                engine="openpyxl",
                index=False,
