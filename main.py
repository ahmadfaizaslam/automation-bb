...
if __name__ == "__main__":
    ...
    master_BC_GCIF = pd.concat(master_BC_GCIF,sort=False).drop_duplicates(subset='Account_No')
    master_BC_GCIF[['BC_NAME']] = master_BC_GCIF[['BC_NAME']]\
                            .replace({'JOHOR BARU BC':'JOHOR BAHRU BC'})\
                            .replace({'SUBANG':'SUBANG BC'})
                            
    master_BC_Acc = pd.concat(master_BC_Acc,sort=False)
    master_BC_Acc[['BC_NAME']] = master_BC_Acc[['BC_NAME']]\
                            .replace({'JOHOR BARU BC':'JOHOR BAHRU BC'})\
                            .replace({'SUBANG':'SUBANG BC'})
    ...
    masterbb = my_transformation.do_merge(master,gil,'GCIF','GCIF #')\
                .drop(columns=['GCIF #']).drop_duplicates() #ok
    masterbb['PL IPL IPL R&R NPL']= masterbb['PL IPL IPL R&R NPL'].replace(np.nan,'PL') #ok
    ...
    lb_fm2 = lb_fm2[['M_Cus_No','NOB','Broad Code','NOB Desc_x','Sub Sector Desc_x','NOB Desc_y']]\
                .rename(columns={'NOB Desc_x':'NOB Desc','Sub Sector Desc_x':'Subsector','NOB Desc_y': 'Broad Sector'})
    """merge lb notfunded with nob"""                 
    lb_ufm = my_transformation.do_merge(lb_uf,nob,'NOB','NOB Code').dropna(how='all')  #ok
    ...
    lb_ufm2 = lb_ufm2[['M_Cus_No','NOB','Broad Code','NOB Desc_x','Sub Sector Desc_x','NOB Desc_y']]\
                .rename(columns={'NOB Desc_x':'NOB Desc','Sub Sector Desc_x':'Subsector','NOB Desc_y': 'Broad Sector'})
    
    lb_fnf = pd.concat([lb_fm2,lb_ufm2],sort=True).drop_duplicates(subset='M_Cus_No') #ok
    ...
    masterbb2 = my_transformation.copy_value(masterbb2,'REGION_x','REGION_y')\
                    .drop(columns=[
                    'M_Cus_No',
                    'Account_No_x',
                    'Account_No_y',
    ...
    masterbb_borr.loc[masterbb_borr['Funded Non Funded'] == 'Funded', 'Funded Balance'] = masterbb_borr['Balance']
    masterbb_borr.loc[masterbb_borr['Funded Non Funded'] == 'Non Funded', 'Non Funded Balance'] = masterbb_borr['Balance']
  
    masterbb_borr1=masterbb_borr.merge(masterbbprep,how='left',left_on='GCIF', right_on='GCIF')
    masterbb_borr1.to_excel(path+r"\\MasterbbBORR_final.xlsx",engine='openpyxl',index=False)
